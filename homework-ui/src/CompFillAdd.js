...
class SubtractionList extends React.Component {
  ...
  getQuestions() {
    ...
    console.log(upper);
    console.log(size);
    axios.get(`http://homestudy.duckdns.org:18200/add?from=${lower}&to=${upper}&size=${size}`)
      .then(res => {
  ...
  trylocalUrl(lower, upper, size) {
    axios.get(`http://homestudy.duckdns.org:8989/add?from=${lower}&to=${upper}&size=${size}`)
    .then(res => {
      const questions = res.data.questions;
      this.setState({ questions });
      console.log(questions);
  ...
  render() {
    return (
      <Typography>
      <Box>
        <Typography variant="h4">Fill In Addition</Typography>
        <Typography>
          <Timer />
        </Typography>
      ...
      <Typography id="inputBlock">
            <Typography variant="h6" noWrap>
              Fill in Addition Configurations
            </Typography>
